...
const WardDashboard = () => {
    ...
    const { toast, user, darkMode, language, issues, projects, selectedProjectId, view, setView, fetchWorkspaceData, currentWorkspace } = useContext(AppContext);
    ...
    useEffect(() => {
        if (wardId) fetchWorkspaceData(wardId);
    }, [wardId]);
    ...
    return (
        <div className={`min-h-screen transition-colors duration-500 ${darkMode ? 'dark bg-navy-950' : 'bg-stone-50'}`}>
            ...
            <main className="relative">
                <AnimatePresence mode="wait">
                    {view === 'map' ? (
                        ...
                        >
                            <Hero />
                            <AdminPortal onOpenGenesis={handleOpenGenesis} onOpenWorkspaceEdit={handleOpenWorkspaceEdit} />
                            <WardHub />
            ...
            <AnimatePresence>
                ...
                {isWorkspaceEditOpen && <WorkspaceEditModal isOpen={isWorkspaceEditOpen} onClose={() => setIsWorkspaceEditOpen(false)} />}
            </AnimatePresence>
            <WardAI />
            ...
            <footer className="bg-navy-950 py-16 border-t border-navy-900 border-dashed">
                <div className="max-w-7xl mx-auto px-6 text-center">
                    <div className="flex items-center justify-center gap-4 mb-8">
                        <div className="w-10 h-10 bg-saffron-500 rounded-xl flex items-center justify-center text-navy-900 font-black text-xs shadow-2xl shadow-saffron-500/30">
                            {currentWorkspace ? (currentWorkspace.ward_name.replace(/[^0-9]/g, '') || currentWorkspace.ward_name.charAt(0)) : '14'}
                        </div>
                        <div className="text-left">
                            <span className="block font-display font-black text-stone-100 text-sm tracking-tight uppercase">
                                {currentWorkspace ? `${currentWorkspace.city} Civic` : 'Ward Civic'}
                            </span>
