...
const ProjectDetails = ({ projectId, onBack }) => {
    const { projects, user, updateProject, showToast } = useContext(AppContext);
    ...
    const handleSave = async () => {
        ...
        if (success) {
            setIsEditing(false);
            showToast("System Node Updated Successfully", "success");
        }
        setLoading(false);
    };
    ...
    const allocationItems = Object.entries(editForm?.allocation_details || {
        "Base Materials": 45,
        "Heavy Machinery": 25,
        "Human Intelligence (Labor)": 20,
        "Strategic Logistics": 10
    });
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6">
                ...
                <div className="flex items-center justify-between mb-12">
                    <button onClick={onBack} className="flex items-center gap-2 text-stone-500 hover:text-navy-900 dark:hover:text-stone-100 transition-all font-mono text-[10px] uppercase tracking-widest font-black">
                        <ChevronLeft className="w-4 h-4" /> Return to Map Matrix
                    </button>
                    {user?.role === 'Authority' && (
                        <button 
                        ...
                        >
                            ...
                            {isEditing ? "Synchronize Changes" : "Activate Edit Protocol"}
                        </button>
                    )}
                </div>
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <span className="text-stone-400 font-mono text-[9px] uppercase tracking-widest flex items-center gap-2">
                                        <HardHat className="w-3.5 h-3.5" /> {project.dept_en}
                                    </span>
                                </div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 py-10 border-y border-stone-100 dark:border-navy-800">
                                    <StatNode label="Approved Budget" value={`₹ ${project.budget} L`} icon={<IndianRupee />} />
                                    <StatNode label="Fund Utilization" value={`₹ ${project.fund_usage || 0} L`} icon={<TrendingUp />} color="text-emerald-500" />
                                    <StatNode label="Target Deadline" value={new Date(project.deadline).toLocaleDateString()} icon={<Calendar />} />
                                    <StatNode label="Security Node" value="Verified" icon={<CheckCircle2 />} color="text-amber-500" />
                                </div>

                                <div className="mt-12">
                                    <h3 className="text-xs font-mono font-black text-stone-400 uppercase tracking-[0.3em] mb-8 flex items-center gap-3">
                                        <PieChart className="w-4 h-4 text-saffron-500" /> Strategic Resource Allocation Breakdown
                                    </h3>
                                        ...
                                        {allocationItems.map(([item, percent]) => (
                                            <div key={item} className="space-y-3">
                                                <div className="flex justify-between items-end">
                                                    ...
                                                    <span className="text-xs font-mono font-black text-navy-900 dark:text-saffron-500">{percent}% (₹ {(project.budget * percent / 100).toFixed(2)} L)</span>
                                                </div>
                    ...
                    <div className="space-y-10">
                        <div className="bg-navy-900 rounded-[3rem] p-10 border border-navy-800 shadow-2xl">
                            <h3 className="text-stone-400 font-mono text-[9px] uppercase tracking-widest font-black mb-8 border-b border-navy-800 pb-4 flex items-center justify-between">
                                Operational Status <Download className="w-3 h-3 cursor-pointer hover:text-white" />
                            </h3>
                            
                            <div className="space-y-10">
                                <div className="text-center">
                                    <div className="text-5xl font-display font-black text-stone-50 mb-2">{project.progress}%</div>
                                    <div className="text-[9px] font-mono text-saffron-500 uppercase tracking-widest font-black">Sync Completion</div>
                                </div>

                                {isEditing ? (
                                    <div className="space-y-6">
                                        <div>
                                            <label className={labelClasses}>Update Progress</label>
                                            <input 
                                ...
                                <div className="pt-6 border-t border-navy-800">
                                    <button className="w-full bg-navy-800 hover:bg-navy-700 text-stone-300 py-4 rounded-2xl text-[9px] font-mono font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3">
                                        View Site Image Ledger <ArrowRight className="w-3 h-3" />
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-navy-900 rounded-[3rem] p-10 border border-stone-200 dark:border-navy-800 shadow-2xl">
                            <h3 className="text-stone-400 font-mono text-[9px] uppercase tracking-widest font-black mb-8 flex items-center gap-3">
                                <History className="w-4 h-4" /> Sync Timeline
                            </h3>
                            <div className="space-y-8">
                                <TimelineNode date="12 Mar" title="Audit Phase Complete" desc="Financial reconciliation successful." active />
                                <TimelineNode date="05 Mar" title="Material Dispatch" desc="Asphalt node synchronized at site." />
                                <TimelineNode date="28 Feb" title="Project Commissioned" desc="Authority protocol initiated." />
                            </div>
                        </div>
                    </div>
                </div>
