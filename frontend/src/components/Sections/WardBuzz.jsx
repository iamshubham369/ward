            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
                <div className="flex flex-wrap items-end justify-between gap-6 mb-12">
                    ...
                    <div className="flex items-center gap-3 bg-navy-900/80 p-3 rounded-2xl border border-navy-700 shadow-2xl backdrop-blur-md">
                        ...
                        <span className="text-[10px] font-mono text-emerald-400 uppercase tracking-widest font-black">{t('buzz.live')} — {issues?.length || 0} {t('buzz.protocols')}</span>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3 mb-10 reveal visible">
                    <div className="relative group">
                        ...
                        <select value={filterCat} onChange={e => setFilterCat(e.target.value)} className="bg-navy-900/80 border border-navy-700/50 text-stone-200 text-xs font-mono rounded-xl pl-9 pr-8 py-3 appearance-none outline-none focus:border-saffron-500 cursor-pointer shadow-lg hover:bg-navy-800 transition-all">
                            <option value="all">{t('buzz.cat')}</option>
                            <option value="Roads">{t('buzz.roles')}</option>
                            <option value="Water Supply">{t('buzz.water')}</option>
                            <option value="Electricity">{t('buzz.elec')}</option>
                            <option value="Sanitation">{t('buzz.sanitation')}</option>
                        </select>
                    </div>
                    <div className="relative group">
                        ...
                        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="bg-navy-900/80 border border-navy-700/50 text-stone-200 text-xs font-mono rounded-xl pl-9 pr-8 py-3 appearance-none outline-none focus:border-saffron-500 cursor-pointer shadow-lg hover:bg-navy-800 transition-all">
                            <option value="all">{t('buzz.status')}</option>
                            <option value="submitted">{t('buzz.submitted')}</option>
                            <option value="in progress">{t('buzz.in_progress')}</option>
                            <option value="resolved">{t('buzz.resolved')}</option>
                        </select>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        ...
                        {filteredIssues.length === 0 ? (
                            <div className="bg-navy-900/50 border-2 border-dashed border-navy-700 p-20 rounded-3xl text-center flex flex-col items-center justify-center grayscale opacity-50">
                                ...
                                <h3 className="font-mono text-sm font-black text-stone-500 uppercase tracking-widest">{t('buzz.no_records')}</h3>
                            </div>
                        ) : (
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="bg-navy-900/80 border border-navy-700 p-8 rounded-3xl sticky top-24 backdrop-blur-xl shadow-2xl group overflow-hidden">
                            ...
                            <h3 className="font-mono text-[10px] font-black text-saffron-400 uppercase tracking-[0.4em] mb-8 flex items-center justify-between">
                                {t('buzz.highlights')} <TrendingUp className="w-4 h-4 text-emerald-400 animate-bounce" />
                            </h3>
                            <div className="space-y-6">
                                <TrendingItem title={t('buzz.h1_title')} count={t('buzz.h1_count')} icon={< Siren className="w-3 h-3" />} />
                                <TrendingItem title={t('buzz.h2_title')} count={t('buzz.h2_count')} icon={< Siren className="w-3 h-3" />} />
                                <TrendingItem title={t('buzz.h3_title')} count={t('buzz.h3_count')} icon={< CheckCircle2 className="w-3 h-3" />} />
                            </div>
                            <button className="mt-10 w-full bg-navy-800 border border-navy-700 hover:border-saffron-500/50 py-4 rounded-xl text-[10px] font-mono font-black text-stone-400 hover:text-saffron-400 transition-all uppercase tracking-widest flex items-center justify-center gap-2 group/btn">
                                {t('buzz.report')} <ArrowRight className="w-3 h-3 group-hover/btn:translate-x-1 transition-transform" />
                            </button>
                        </div>
                    </div>
                </div>
...
const IssueCard = ({ issue, onUpvote, onStatus, isAdmin }) => {
    ...
    return (
        <div className={`bg-navy-900/80 border rounded-3xl p-6 sm:p-8 hover:translate-y-[-4px] transition-all duration-500 group relative backdrop-blur-xl shadow-2xl overflow-hidden ${isResolved ? 'border-emerald-500/30' : 'border-navy-700 hover:border-saffron-500/30'}`}>
            {isResolved && <div className="absolute top-4 right-4 bg-emerald-500/10 text-emerald-500 border border-emerald-500/50 px-3 py-1.5 rounded-xl font-mono text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 z-10"><CheckCircle2 className="w-3 h-3" /> {t('buzz.verified')}</div>}
            ...
            <div className="flex flex-col sm:flex-row gap-6 relative z-10">
                <div className="flex flex-col items-center gap-2 shrink-0">
                    ...
                    <span className="text-[8px] font-mono font-bold text-stone-500 uppercase tracking-widest mt-1">{t('buzz.upvotes')}</span>
                </div>

                <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-3 mb-3">
                        <span className="bg-navy-800 border border-navy-700 text-saffron-500 px-3 py-1 rounded-xl text-[9px] font-mono font-black uppercase tracking-widest">{t(`cat.${(issue.category || '').toLowerCase().replace(' ', '_')}`)}</span>
                        <span className="text-[10px] font-mono text-stone-500 uppercase tracking-widest font-bold">{t('buzz.protocol')}: <span className="text-stone-300">{issue.id}</span></span>
                    </div>
                    <h3 className="text-xl font-black text-stone-100 mb-3 leading-tight tracking-tight uppercase group-hover:text-saffron-400 transition-colors">
                        {language === 'hi' ? issue.description_hi : language === 'mr' ? issue.description_mr : issue.description}
                    </h3>
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[10px] text-stone-400 font-mono font-bold uppercase tracking-widest">
                        <span className="flex items-center gap-2">
                            <MapPin className="w-3.5 h-3.5 text-saffron-500/50" /> 
                            {language === 'hi' ? issue.street_hi : language === 'mr' ? issue.street_mr : issue.street}
                            {issue.landmark && ` — ${language === 'hi' ? issue.landmark_hi : language === 'mr' ? issue.landmark_mr : issue.landmark}`}
                        </span>
                        <span className="flex items-center gap-2"><Calendar className="w-3.5 h-3.5 text-saffron-500/50" /> {new Date(issue.timestamp).toLocaleDateString(language === 'hi' ? 'hi-IN' : language === 'mr' ? 'mr-IN' : 'en-IN', { day:'numeric', month:'short' })}</span>
                        <span className="flex items-center gap-2"><MessageSquare className="w-3.5 h-3.5 text-saffron-500/50" /> {Math.floor(issue.upvotes / 3)} {t('buzz.comms')}</span>
                    </div>

                    {isAdmin && !isResolved && (
                        <div className="mt-8 pt-6 border-t border-navy-700 flex flex-wrap gap-2">
                            <button onClick={() => onStatus(issue.id, 'In Progress')} className="bg-navy-800 border border-navy-700 hover:border-saffron-500/50 px-4 py-2.5 rounded-xl text-[9px] font-mono font-black text-stone-400 hover:text-saffron-400 transition-all uppercase tracking-widest">{t('buzz.mark_progress')}</button>
                            <button onClick={() => onStatus(issue.id, 'Resolved')} className="bg-emerald-500/10 border border-emerald-500/30 hover:bg-emerald-500 hover:text-navy-900 px-4 py-2.5 rounded-xl text-[9px] font-mono font-black text-emerald-500 hover:text-navy-900 transition-all uppercase tracking-widest">{t('buzz.verify_resolved')}</button>
                        </div>
                    )}
                </div>
