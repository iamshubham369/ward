...
const ReportIssue = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setTrackingId(tid);
            showToast(`Protocol Active: Issue ${tid} Logged`, 'success');
            fetchIssues();
            setFormData({
                category: '', street: '', landmark: '', pincode: '', description: '', priority: '', anonymous: false, file: null
            });
        } catch (err) {
            showToast('System Error: Synchronization Failed', 'error');
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <div className="lg:col-span-7 bg-white dark:bg-navy-800 rounded-3xl border border-stone-200 dark:border-navy-700 p-8 sm:p-12 shadow-2xl relative overflow-hidden group">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                            ...
                            <div className="grid sm:grid-cols-2 gap-6">
                                <div><label className="block text-xs font-mono font-bold text-navy-800 dark:text-stone-400 uppercase tracking-widest mb-2">Street / Location <span className="text-red-400">*</span></label><input type="text" value={formData.street} onChange={e => setFormData({...formData, street: e.target.value})} placeholder="e.g. MG Road, near crossing" className="w-full px-4 py-3 rounded-xl border border-stone-200 dark:border-navy-700 dark:bg-navy-900 text-sm focus:border-saffron-500 outline-none transition-all dark:text-stone-100" /></div>
                                <div><label className="block text-xs font-mono font-bold text-navy-800 dark:text-stone-400 uppercase tracking-widest mb-2">Landmark</label><input type="text" value={formData.landmark} onChange={e => setFormData({...formData, landmark: e.target.value})} placeholder="e.g. Opposite Mall" className="w-full px-4 py-3 rounded-xl border border-stone-200 dark:border-navy-700 dark:bg-navy-900 text-sm focus:border-saffron-500 outline-none transition-all dark:text-stone-100" /></div>
                            </div>

                            <div><label className="block text-xs font-mono font-bold text-navy-800 dark:text-stone-400 uppercase tracking-widest mb-2">Issue Description <span className="text-red-400">*</span></label><textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} rows="4" placeholder="Detailed intelligence about the issue..." className="w-full px-4 py-3 rounded-xl border border-stone-200 dark:border-navy-700 dark:bg-navy-900 text-sm focus:border-saffron-500 outline-none transition-all dark:text-stone-100 resize-none"></textarea></div>

                            <div className="grid sm:grid-cols-2 gap-6 items-end">
                                <div>
                                    <label className="block text-xs font-mono font-bold text-navy-800 dark:text-stone-400 uppercase tracking-widest mb-2">Attach Media</label>
                                    <label className="flex items-center gap-3 px-4 py-3 rounded-xl border-2 border-dashed border-stone-200 dark:border-navy-700 hover:border-saffron-400 dark:hover:border-saffron-500 cursor-pointer transition-all bg-stone-50 dark:bg-navy-900 group/upload">
                                        ...
                                        <span className="text-xs text-stone-500 truncate">{formData.file ? formData.file.name : 'Click to upload payload'}</span>
                                ...
                                <div id="mini-map-trigger" className="w-full px-4 py-3 rounded-xl border border-stone-200 dark:border-navy-700 bg-stone-50 dark:bg-navy-900 flex items-center justify-center gap-2 cursor-pointer hover:border-saffron-500/50 group/map transition-all">
                                    ...
                                    <span className="text-xs font-mono text-stone-500 uppercase tracking-widest font-bold">Pin Telemetry</span>
                                </div>
                            </div>

                            <div>
                                <label className="block text-xs font-mono font-bold text-navy-800 dark:text-stone-400 uppercase tracking-widest mb-4">Priority Level <span className="text-red-400">*</span></label>
                            ...
                            <div className="flex items-center justify-between p-4 bg-stone-50 dark:bg-navy-900/50 rounded-2xl border border-stone-200 dark:border-navy-700">
                                <div>
                                    <span className="text-xs font-bold text-navy-900 dark:text-stone-200 uppercase tracking-widest">Protocol: Anonymous</span>
                                    <p className="text-[10px] text-stone-400 mt-1 uppercase">Your identity is scrubbed from public view</p>
                                </div>
                            ...
                            <button type="submit" disabled={loading} className="w-full bg-saffron-500 hover:bg-saffron-400 text-navy-900 font-black py-5 rounded-2xl text-xs font-mono uppercase tracking-[0.2em] transition-all shadow-2xl shadow-saffron-500/20 active:scale-[0.99] flex items-center justify-center gap-3">
                                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Shield className="w-4 h-4" /> AUTHORIZE BROADCAST</>}
                            </button>
                        </form>
                    </div>
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        {!trackingId ? (
                            <div className="bg-white dark:bg-navy-800 rounded-3xl border border-stone-200 dark:border-navy-700 p-10 shadow-xl h-full flex flex-col items-center justify-center text-center group">
                                ...
                                <h3 className="font-display font-black text-2xl text-navy-900 dark:text-stone-50 mb-4 tracking-tight">Active Tracking</h3>
                                <p className="text-sm font-mono text-stone-500 uppercase tracking-widest mb-10 max-w-[280px] mx-auto">Waiting for protocol initiation...</p>
                            ...
                            <div className="bg-white dark:bg-navy-800 rounded-3xl border-2 border-saffron-500 p-10 shadow-2xl animate-in zoom-in slide-in-from-right-4 duration-500">
                                ...
                                <div className="space-y-8">
                                    <TrackerStep icon={<FileText className="w-4 h-4" />} title="Broadcast Authenticated" desc="Record created in ward ledger" active time="Just now" />
                                    <TrackerStep icon={<Search className="w-4 h-4" />} title="Under Intelligence Review" desc="Ward 14 office verifying telemetry" />
                                    <TrackerStep icon={<Wrench className="w-4 h-4" />} title="Operations Assigned" desc="Strategic deployment scheduled" />
                                    <TrackerStep icon={<CircleCheck className="w-4 h-4" />} title="Resolution Finalized" desc="Issue logged as resolved" last />
                                </div>
                                <button onClick={() => setTrackingId(null)} className="mt-10 w-full py-4 text-[10px] font-mono font-black text-stone-400 hover:text-navy-900 dark:hover:text-stone-100 uppercase tracking-widest transition-colors flex items-center justify-center gap-2">
                                    Dismiss Session <Loader2 className="w-3 h-3" />
                                </button>
                            </div>
                        )}
                    </div>
